...
# <a name="lab---package-azure-virtual-desktop-applications-ad-ds"></a>Laboratorio: Empaquetado de aplicaciones de Azure Virtual Desktop (AD DS)
...
- Implementar una imagen de conexión de aplicaciones MSIX para Azure Virtual Desktop en un entorno de AD DS
...
### <a name="exercise-1-prepare-for-and-create-msix-app-packages"></a>Ejercicio 1: Preparar y crear paquetes de aplicaciones MSIX
...
#### <a name="task-1-prepare-for-configuration-of-azure-virtual-desktop-session-hosts"></a>Tarea 1: Preparar la configuración de hosts de sesión de Azure Virtual Desktop
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para iniciar las máquinas virtuales de Azure del host de sesión de Azure Virtual Desktop que vamos a usar en este laboratorio:
   ...
   >**Nota**: Continúe directamente con la tarea siguiente sin esperar a que se inicien las máquinas virtuales de Azure.

#### <a name="task-2-deploy-an-azure-vm-running-windows-10-by-using-an-azure-resource-manager-quickstart-template"></a>Tarea 2: Implementar una máquina virtual de Azure que ejecuta Windows 10 mediante una plantilla de inicio rápido de Azure Resource Manager
...
1. En la sesión de PowerShell del panel de Cloud Shell, ejecute lo siguiente para implementar una máquina virtual de Azure que ejecute Windows 10, que se usará para crear paquetes MSIX y unirla al dominio de Azure AD DS:
...
#### <a name="task-3-prepare-the-azure-vm-running-windows-10-for-msix-packaging"></a>Tarea 3: Preparar la máquina virtual de Azure que ejecuta Windows 10 para el empaquetado MSIX
...
1. Cuando se le solicite, inicie sesión con el nombre de usuario **wvdadmin1** y la contraseña que estableció al crear esta cuenta de usuario. 
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie **Windows PowerShell ISE** como administrador y, en la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente a fin de preparar el sistema operativo para el empaquetado de MSIX:
...
#### <a name="task-4-generate-a-signing-certificate"></a>Tarea 4: Generación de un certificado de firma
...
#### <a name="task-5-download-software-to-package"></a>Tarea 5: Descargar software para empaquetarlo

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie **Microsoft Edge** y vaya a **https://github.com/microsoft/XmlNotepad** .
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie Explorador de archivos, vaya a la carpeta **Descargas**, abra el archivo comprimido, copie el contenido de la carpeta del archivo comprimido y péguelo en el directorio **C:\\AllFiles\\Labs\\04\\** . 

#### <a name="task-6-install-the-msix-packaging-tool"></a>Tarea 6: Instalar la herramienta de empaquetado MSIX

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie la aplicación de **Microsoft Edge**.
...
#### <a name="task-7-create-an-msix-package"></a>Tarea 7: Crear un paquete MSIX

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, cambie a la ventana **Administrador: Windows PowerShell ISE** y, en el panel del script de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente para deshabilitar el servicio de Windows Search:
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente para crear la carpeta que hospedará el paquete MSIX:

   ```powershell
   New-Item -ItemType Directory -Path 'C:\AllFiles\Labs\04\XmlNotepad' -Force
   ```

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente para quitar el flujo de datos alternativo Zone.Identifier de los archivos del instalador extraídos, que tiene un valor de "3" para indicar que se descargaron desde Internet:

   ```powershell
   Get-ChildItem -Path 'C:\AllFiles\Labs\04' -Recurse -File | Unblock-File
   ```

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, cambie a la interfaz **MSIX Packaging Tool** (Herramienta de empaquetado MSIX), en la página **Seleccionar tarea**, seleccione la entrada **Application package - Create your app package** (Paquete de aplicación: Crear el paquete de la aplicación). Se iniciará el Asistente para **crear un paquete**.
...
1. En la página **Preparar equipo** del Asistente para **crear un paquete**, revise las recomendaciones. Si hay un reinicio pendiente, reinicie el sistema operativo, vuelva a iniciar sesión con la cuenta **ADATUM\wvdadmin1** y reinicie la **Herramienta de empaquetado MSIX** antes de continuar. 
...
### <a name="exercise-2-implement-an-msix-app-attach-image-for-azure-virtual-desktop-in-azure-ad-ds-environment"></a>Ejercicio 2: Implementar una imagen de conexión de aplicaciones MSIX para Azure Virtual Desktop en un entorno de AD DS
...
#### <a name="task-1-enable-hyper-v-on-the-azure-vms-running-window-10-enterprise-edition"></a>Tarea 1: Habilitar Hyper-V en las máquinas virtuales de Azure que ejecutan Windows 10 Enterprise Edition

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente a fin de preparar los hosts de Azure Virtual Desktop de destino para la conexión de aplicaciones MSIX: 
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente para instalar Hyper-V y sus herramientas de administración, incluido el módulo de PowerShell de Hyper-V en los hosts de Azure Virtual Desktop:
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente para instalar Hyper-V y sus herramientas de administración, incluido el módulo de PowerShell de Hyper-V en el equipo local:

   ```powershell
   Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V-All
   ```

1. Una vez completada la instalación de los componentes de Hyper-V, seleccione **Sí** para reiniciar el sistema operativo. Después del reinicio, vuelva a iniciar sesión mediante la cuenta **wvdadmin1** con la contraseña **Pa55w.rd1234**.

#### <a name="task-2-create-an-msix-app-attach-image"></a>Tarea 2: Crear una imagen de conexión de aplicaciones MSIX

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie **Microsoft Edge** y vaya a **https://aka.ms/msixmgr** . Esto descargará automáticamente el archivo **msixmgr.zip** (el archivo de herramientas mgr MSIX) en la carpeta **Descargas**.
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie **Windows PowerShell ISE** como administrador y, en el panel del script de **Administrador: Windows PowerShell script ISE**, ejecute lo siguiente para crear el archivo VHD que servirá como imagen de conexión de aplicaciones MSIX:
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, en Explorador de archivos, vaya a la carpeta **F:\\Apps** y revise su contenido. Si se le pide que obtenga acceso a la carpeta, seleccione **Continuar**.
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell script ISE**, ejecute lo siguiente para desmontar el archivo VHD que servirá como imagen de MSIX:

   ```powershell
   Dismount-VHD -Path "C:\Allfiles\Labs\04\MSIXVhds\$appName.vhd" -Confirm:$false
   ```

### <a name="exercise-3-implement-msix-app-attach-on-azure-virtual-desktop-session-hosts"></a>Ejercicio 3: Implementación de conexión de aplicaciones MSIX en hosts de sesión de Azure Virtual Desktop
...
#### <a name="task-1-configure-active-directory-groups-containing-azure-virtual-desktop-hosts"></a>Tarea 1: Configurar grupos de Active Directory que incluyan hosts de Azure Virtual Desktop
...
1. Desde el panel del script de**Administrador: Windows PowerShell ISE**, ejecute lo siguiente para crear un objeto de grupo de AD DS que se sincronizará con el inquilino de Azure AD usado en este laboratorio:
...
1. En la página **Conectar a Azure AD** de la ventana **Microsoft Azure Active Directory Connect**, autentíquese mediante el nombre principal de usuario de la cuenta de usuario **aadsyncuser** que identificó anteriormente en esta tarea con la contraseña que estableció al crear esta cuenta de usuario.
...
#### <a name="task-2-set-up-the-azure-files-share-for-msix-app-attach"></a>Tarea 2: Configurar el recurso compartido de Azure Files para la conexión de aplicaciones MSIX

1. En el equipo de laboratorio, cambie a la sesión de Escritorio remoto a **az140-cl-vm42**.
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie Microsoft Edge en el modo InPrivate, vaya a [Azure Portal](https://portal.azure.com) e inicie sesión proporcionando credenciales de una cuenta de usuario con el rol Propietario en la suscripción que va a usar en este laboratorio.

   > **Nota**: Asegúrese de usar el modo InPrivate de Microsoft Edge.

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, en la ventana de Microsoft Edge en la que se muestra Azure Portal, busque y seleccione **Cuentas de almacenamiento** y, en la hoja **Cuentas de almacenamiento**, seleccione la cuenta de almacenamiento que configuró para hospedar los perfiles de usuario.

   > **Nota**: Esta parte del laboratorio depende de completar el laboratorio **Administración de perfiles de Azure Virtual Desktop (AD DS)** o **Administración de perfiles de Azure Virtual Desktop (Azure AD DS)** .

   > **Nota**: En escenarios de producción, debe plantearse usar una cuenta de almacenamiento independiente. Esto requeriría configurar esa cuenta de almacenamiento para la autenticación de Azure AD DS, que ya implementó para la cuenta de almacenamiento que hospeda perfiles de usuario. Está usando la misma cuenta de almacenamiento para minimizar los pasos duplicados en laboratorios individuales.
...
1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie **Símbolo del sistema** y, desde la ventana **Símbolo del sistema**, ejecute lo siguiente para asignar una unidad al recurso compartido **az140-42-msixvhds** (reemplace el marcador de posición `<storage-account-name>` por el nombre de la cuenta de almacenamiento) y compruebe que el comando se completa correctamente:

   ```cmd
   net use Z: \\<storage-account-name>.file.core.windows.net\az140-42-msixvhds
   ```

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la ventana **Símbolo del sistema**, ejecute lo siguiente para conceder los permisos NTFS necesarios a las cuentas de equipo de los hosts de sesión:
   ...
   >**Nota**: También puede establecer estos permisos mediante Explorador de archivos durante el inicio de sesión como **ADATUM\\wvdadmin1**. 

   >**Nota**: A continuación validará la función de la conexión de aplicaciones MSIX.

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, desde la consola de **Administrador: Windows PowerShell ISE**, ejecute lo siguiente para copiar el archivo VHD que creó en el ejercicio anterior al recurso compartido de Azure Files que creó anteriormente en este ejercicio:
...
#### <a name="task-3-mount-and-register-the-msix-app-attach-image-on-azure-virtual-desktop-session-hosts"></a>Tarea 3: Montar y registrar de a imagen de conexión de aplicaciones MSIX en hosts de sesión de Azure Virtual Desktop

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, en la ventana de Microsoft Edge en la que se muestra Azure Portal, busque y seleccione **Azure Virtual Desktop** y, en la hoja **Azure Virtual Desktop**, en el menú vertical de la izquierda, en la sección **Administrar**, seleccione **Grupos de hosts**.
...
#### <a name="task-4-publish-msix-apps-to-an-application-group"></a>Tarea 4: Publicar de aplicaciones MSIX en un grupo de aplicaciones

> **Nota**: Publicará la aplicación MSIX en la aplicación remota y en el grupo de aplicaciones de escritorio. 

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, en la ventana de Microsoft Edge en la que se muestra Azure Portal, busque y seleccione **Azure Virtual Desktop** y, en la hoja **Azure Virtual Desktop**, en el menú vertical de la izquierda, en la sección **Administrar**, seleccione **Grupos de aplicaciones**.
...
1. En la hoja **Agregar aplicaciones**, especifique la configuración siguiente y seleccione **Guardar**:
   ...
   |Ruta de icono|**C:\Program Files\WindowsApps\XmlNotepad_2.8.0.0_x64__4vm7ty4fw38e8\VFS\ProgramFilesX86\LovettSoftware\XmlNotepad\XmlNotepad.exe**|
   |Índice de icono|**0**|
...
#### <a name="task-5-validate-the-functionality-of-msix-app-attach"></a>Tarea 5: Validar la función de la conexión de aplicaciones MSIX

1. En la sesión de Escritorio remoto a **az140-cl-vm42**, inicie Microsoft Edge y vaya a la [página de descarga del cliente de escritorio de Windows](https://go.microsoft.com/fwlink/?linkid=2068602); cuando se complete la descarga, seleccione **Abrir archivo** para iniciar su instalación. En la página **Ámbito de instalación** del asistente de **Instalación de Escritorio remoto**, seleccione la opción **Instalar para todos los usuarios de esta máquina** y haga clic en **Instalar**. 
...
### <a name="exercise-4-stop-and-deallocate-azure-vms-provisioned-and-used-in-the-lab"></a>Ejercicio 4: Detención y desasignación de las máquinas virtuales de Azure aprovisionadas en el laboratorio
